...
def game_win():
    ...
    win_inv = [item_id, item_laptop, item_money, item_biscuits, item_pen, item_handbook]
    ...
    if current_room == rooms["Reception"] and win_inv_count == len(win_inv):
        print("")
        print("")
        print(colored("YOU WIN!", "cyan", "bold"))
        print("")
        print("(Such a shame that the biscuits aren't real though...)")
        return True


def name_validation(input_text):
...
def get_name():
    global name
    while name[0] == "":
        f_name = input("Please input your first name: ").strip()
        if name_validation(f_name):
            name[0] = f_name

    while name[1] == "":
        s_name = input("Please input your last name: ").strip()
        if name_validation(s_name):
            name[1] = s_name
